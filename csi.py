...
def get_all_squares_and_measure(contours, scale):
    """获取所有正方形并返回其边长列表"""
    squares = []
    for cnt in contours:
        ...
        if is_sq:
            ...
            if side_length > 0:
                squares.append((cnt, side_length))
    return squares
...
def detect_whiteA4(img, edges):
    ...
    return frame, rect_image, roi_position, scale, distance
...
def draw_contours_on_frame(frame_bgr, roi_contours, roi_pos, mode, scale, distance):
    ...
    global all_square_sides, current_measured_shape, L, result_sent
    ...
    if mode == "simple":
        for cnt in roi_contours:
            ...
            if shape == "Circle" and scale > 0:
                ...
                if is_measuring:
                    square_measurements.append(diameter)
                ...
                for i in range(min(num_points, DISPLAY_EDGE_LIMIT)):
                    ...
                    if is_measuring:
                        square_measurements.append(actual_length)
    ...
    # 复杂模式处理（根据用户需求调整）
        ...
        if len(squares) > 0:
            if len(squares) == 1:
                ...
                cv2.putText(frame_bgr, "Square", (x + 10, y - 10),
                ...
                if is_measuring and side_length > 0:
                    square_measurements.append(side_length)
            else:
                shape = "Multiple Squares"
                # 获取所有正方形及其边长
                square_data = get_all_squares_and_measure(squares, scale)
                for i, (cnt, side_length) in enumerate(square_data):
                    cnt_in_frame = cnt + [x_offset, y_offset]
                    cv2.drawContours(frame_bgr, [cnt_in_frame], -1, (0, 255, 0), 2)
                    x, y, w, h = cv2.boundingRect(cnt_in_frame)
                    cv2.putText(frame_bgr, f"Square {i+1}: {side_length:.2f}cm", 
                ...
                # 标记最小正方形
                main_contour = find_smallest_square(squares)
    ...
    if is_measuring and main_contour is not None and scale > 0:
        ...
        if elapsed <= MEASUREMENT_DURATION:
            ...
            if filtered_data:
                # 存储所有检测到的正方形边长
                all_square_sides.extend(filtered_data)
                # 取最小值作为最终结果
                measurement_result = min(filtered_data)
            else:
                measurement_result = None
                all_square_sides = []  # 清空无效数据
            
            L = measurement_result
            result_sent = False  # 重置发送状态
            
            # 打印测量信息
            if measurement_result is not None and D is not None:
                print(f"测量完成: 最小正方形边长 = {measurement_result:.2f}cm")
                print(f"所有检测到的正方形边长: {[round(s, 2) for s in all_square_sides]}")
            else:
    ...
    if measurement_result is not None:
        # 显示最小边长
        cv2.putText(frame_bgr, f" X: {measurement_result:.2f}cm", 
                   (20, 500), FONT, FONT_SIZE_MEDIUM, COLOR_MEASUREMENT_RESULT, 2)
        # 显示距离
        cv2.putText(frame_bgr, f" D: {D if D is not None else 'N/A'}cm", 
                   (20, 520), FONT, FONT_SIZE_MEDIUM, COLOR_MEASUREMENT_RESULT, 2)
        # 显示检测到的正方形数量
        if all_square_sides:
            cv2.putText(frame_bgr, f" {len(all_square_sides)} Square", 
                       (20, 545), FONT, FONT_SIZE_MEDIUM, COLOR_MEASUREMENT_RESULT, 2)
...
def start_measurement():
    ...
    global all_square_sides, result_sent
    is_measuring = True
    measure_start_time = time.time()
    square_measurements = []  # 清空当前测量周期的边长数据
    # 保留历史数据但不清除，如需每次测量重新开始可取消注释
    # all_square_sides = []  
    measurement_result = None
    result_sent = False  # 重置发送状态
    print("开始测量...")
...
def serial_communication_loop(ser):
    ...
    while True:
        ...
        if measurement_result is not None and D is not None and not result_sent and not is_measuring:
            # 构建发送字符串，只输出最小边长，其他边长在内部存储
            if current_mode == "simple":
                send_str = f"L:{measurement_result:.2f}cm,D:{D}cm\n"
            else:  # complex mode
                send_str = f"L:{measurement_result:.2f}cm,D:{D}cm\n"
            ...
            if send_data(ser, send_str):
                ...
                print(f"内部存储的所有正方形边长: {[round(s, 2) for s in all_square_sides]}")
        ...
        if received:
            with mode_lock:  # 确保模式切换线程安全
                if received == "1":
                    current_mode = "simple"
                    send_data(ser, "Simple measurement\n")
                    start_measurement()
                elif received == "2":
                    current_mode = "complex"
                    send_data(ser, "Complex measurement\n")
                    start_measurement()
                elif received == "q":
...
def main():
    ...
    # 初始化串口
    ser = init_serial(port='/dev/ttyAMA0', baudrate=9600)
    ...
    try:
        while True:
            ...
            frame_bgr, roi, roi_pos, scale, distance = detect_whiteA4(frame_bgr, edges)
            ...
            frame_bgr = draw_contours_on_frame(frame_bgr, roi_contours, roi_pos, current_mode_copy, scale, distance)
